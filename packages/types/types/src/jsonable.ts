...
export type JsonableObject<T = void> = {
    [key: string]: Jsonable<T>
    [key: number]: Jsonable<T>
};

export type JsonableArray<T = void> = Jsonable<T>[];
 ...
 * (e.g., `Jsonable<Map>` allows using a replacer to serialize 'Map' instances.)
 *
 ...
 */
export type Jsonable<T = void> = T | JsonablePrimitive | JsonableArray<T> | JsonableObject<T>;
